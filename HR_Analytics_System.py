...
# Load data
def load_data():
    df_deployment = pd.read_csv('Q3_Deployment_Used_Extended_Dataset_with_Resignation.csv')
...
def employee_performance():
    ...
    # Create Quarter string like 2025'Q1
    ...
    # Safely convert to Quarter_Period
    df['Quarter_Period'] = pd.to_datetime(
        df['Quarter'].str[:4] + '-' + df['Quarter'].str[5:], 
        errors='coerce'  # invalid formats become NaT
    ).dt.to_period('Q')

    # Filter only up to current quarter
